...
import {APPS_PER_PAGE} from "../../ReactConstants";
...
export const getDisplayedApps = (pageNum: number, itemsPerPage: number, filter?: string) : getDisplayedAppsObj => {
    ...
    if(IS_DEBUG){
        ...
        console.log(`getDisplayedApps(pageNum: ${pageNum}, itemsPerPage:${itemsPerPage}`, `filter: ${filter}` , res)
...
function AppsCatalogPage(props: AppsCatalogPageProps){
    ...
    const handlePageClick = (arg: { selected:number }) =>{
        ...
        let res: getDisplayedAppsObj = getDisplayedApps(arg.selected, APPS_PER_PAGE)
        setDisplayedApps(res.displayedApps)
        setNumberOfPages(res.pageCount)
    }
